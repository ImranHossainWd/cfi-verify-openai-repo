...
- On an EXTRA CASE - SORTING QUALITY REPORT, inspect the "CASE Metal Detector Verification" table row separately. Only inspect handwriting/checkmarks physically inside the row cells directly below Date, Pallet/Bin #, Passed, Failed, Initials, and Office. Ignore QC, Verification, 2nd Verification, dates, initials, checkmarks, red review marks, and handwriting elsewhere on the form. Set `row_used` true if ANY row cell other than Office contains handwriting or a checkmark. Set it false when those row cells are blank. Set `office_checked` true ONLY when a handwritten mark, checkmark, initials, or signature is physically inside the dotted Office cell in the same used row. The blue printed word "Office:" and marks outside the Office cell do not count. If that dotted cell is empty, set `office_checked` false. If `row_used` is false, return null/empty row values even when other parts of the page are completed.
...

{
  ...
  "original_crop_years": "<original/source crop year(s) if present, else null>",
  "original_line": "<original/source line number if present, else null>",
  "total_defect_pct": <number or null>,
  ...
  "shipping_inspection_items": [{"item": "<Trailer/Cargo inspection item>", "status": "Okay|Fail|Blank|Unreadable"}],
  "shipment_decision": "<Accepted / Rejected / Hold / other decision if present, else null>",
  "comments_corrective_actions": "<comments or corrective-action text if present, else null>",
